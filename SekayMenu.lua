...
if data.ExpireAt and data.ExpireAt ~= "Unknown" then
    local success, expireTime = pcall(function()
        ...
        -- Konversi waktu kedaluwarsa menjadi epoch time.
        -- CATATAN: Jika 'data.ExpireAt' dijamin UTC, hasil dari os.time() ini akan memperhitungkan timezone lokal.
        -- Perbandingannya di bawah akan menggunakan waktu UTC saat ini (os.time(os.date("!*t"))),
        -- yang merupakan cara paling aman untuk perbandingan waktu absolut.
        return os.time({year = y, month = m, day = d, hour = h, min = min, sec = s})
    end)
    
    -- Ambil waktu saat ini secara eksplisit dalam format UTC/absolute epoch time
    local currentTime = os.time(os.date("!*t")) 

    if success and expireTime and expireTime < currentTime then
        pcall(function()
            game.StarterGui:SetCore("SendNotification", {
                Title = "Sekay Hub",
...
local Tabs = {
    ...
    Esp = Window:AddTab("ESP Player", "eye"),
