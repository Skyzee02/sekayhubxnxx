...
-- Daftar Durasi Key (Diambil dari skrip sebelumnya)
local DURATIONS = {
    ["30M"] = 30 * 60,
    ["1D"] = 24 * 60 * 60,
    ["7D"] = 7 * 24 * 60 * 60,
...
local function SendWebhook(data)
    -- ... (Logic SendWebhook tetap sama, tidak ada perubahan UI syntax) ...
...
local function LoadAndCheckKey()
    if isfile(LOCAL_SAVE_FILE) then
        ...
        if success and savedData and savedData.Key and savedData.HWID == hwid then
            ...
            local _, keyData = ValidateKey(key) 
...
-- PERBAIKAN LOGIC: Definisikan CheckRemoteWhitelist (disederhanakan karena logikanya tidak ada)
local function CheckRemoteWhitelist()
    -- Jika ada logic pengecekan admin/owner via user ID atau HWID ke server lain,
    -- letakkan di sini. Tanpa logic, kembalikan false.
    return false
end
